...
export default async function page({params: {locale}, searchParams}) {
  ...
  // const data = await fetchCategories(locale, [...fields, "posts"], []);
        ...
        {categories.map((category) => (
            <div>
                ...
                <div className="grid max-w-2xl grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-16 lg:max-w-none lg:grid-cols-3">
                    {category.posts && category.posts.slice(0,3).map((cate, index) => (
                        <CardCustom index={index} article={cate} lang={locale} time />
                    ))}
                </div>
            </div>
        ))}
